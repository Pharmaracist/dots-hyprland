...
export const ModuleGtkModeToggle = async (props = {}) => {
  const button = Widget.Button({
    ...
    onClicked: async (self) => {
      // Toggle the GTK mode based on current state
      const currentMode = exec(
        `gsettings get org.gnome.desktop.interface gtk-theme`,
      );
      const newMode = currentMode.includes("dark") ? "light" : "dark";

      // Change GTK theme mode
      exec(`gsettings set org.gnome.desktop.interface gtk-theme ${newMode}`);

      // Update button appearance
      self.toggleClassName("sidebar-button-active", newMode === "dark");
    },
