        ...
        setup: (area) => area
            ...
            .on('draw', Lang.bind(area, (area, cr) => {
                ...
                for (let i = 1; i <= count; i++) {
                    ...
                    // Get Unicode label from options
                    const wsOptions = userOptions.asyncGet().workspaces;
                    const wsId = i + offset;
                    let label;
                    if (wsOptions.style === 'unicode') {
                        label = wsOptions.labels[wsId] || wsOptions.defaultLabel || '•';
                    } else {
                        label = wsOptions.numeric_style === 'numeric' ? `${wsId}` : `${i}`;
                    }
